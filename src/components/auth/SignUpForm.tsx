...
export function SignUpForm() {
  const { signUp, sendOtp, verifyOtp, loading, error } = useAuth();
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // 인증번호 관련 에러는 otpError로 처리하여 위쪽에만 표시
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {message && (
        <Alert
          className={
            message.includes("완료") ? "border-green-500 bg-green-50" : ""
          }
        >
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <button
            type="button"
            className="absolute text-gray-400 transform -translate-y-1/2 bg-white border-none right-3 top-1/2 hover:text-gray-600 focus:outline-none"
